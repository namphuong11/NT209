...
int getByte(int x, int n) {
    // Di chuyen byte can lay ve b�n phai de n� tro thanh byte thap nhat
...
int is16x(int x) {
    int res;
    res =  bitAnd(x , 0xF) ; // dung phep AND kiem tra 4 bit thap nhat, neu 4 bit thap nhat  bang 0 thi so do chia het cho 16
    x= !(res);// !0=1 ( chia het cho 16 )
    return x;
}

int isPositive(int x) {
    //ta xet bit dau cua x sau khi dich phai 31 lan
    //ta xet dau cua x( neu x khac 0 thi no tra ve 1, bang 0 thi tra ve 0)
    //sau do ta xor gia tri doi cua moi gia tri ta da lay cho nhau nham lay ket qua nhu de yeu cau
    x= (!(x>>31) ^ !x);
    return x;
}

int isGE2n(int x, int n) {
    x= (x>>(2 << n)) + (-1); // ta se dua x ve dang x/(2^n) - 1 de xet dau ( dua tren bieu thuc da cho ta chia 2 ve cho 2^n rồi -1 de xet no lon hong bang 0 )
    return !isPositive(x); // su dung ham isPositive da co san de xuat duoc ket qua de yeu cau
}
